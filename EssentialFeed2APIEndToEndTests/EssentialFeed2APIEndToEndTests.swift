...
//
//  Created by Ricardo Herrera Petit on 12/30/20.
//
...
class EssentialFeed2APIEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGetresult_matchesFixedTestAccountData() {
        ...
        case let .failure(error)?:
            XCTFail("Expected success got feed result got \(error) instead")
        default:
            XCTFail("Expected successful feed result, got no result intstead")
        }
    }
    
    func test_endToEndServerGETFeedImageDataResult_matchesFixedTestAccountData() {
    ...
    private func getFeedResult(file: StaticString = #file, line:UInt = #line) -> FeedLoader.Result? {
        let loader = RemoteFeedLoader(url: feedTestServerURL, client:   ephimeralClient() )
    ...
    private func getFeedImageDataResult(file: StaticString = #file, line:UInt = #line) -> FeedImageDataLoader.Result? {
        let loader = RemoteFeedImageDataLoader(client: ephimeralClient())
        trackForMemoryLeaks(loader, file: file, line: line)
    ...
    private func ephimeralClient(file: StaticString = #file, line:UInt = #line) -> HTTPClient {
    ...
    private func expectedImage(at index:Int) -> FeedImage {
        return FeedImage(id: id(at: index), description: description(at: index), location: location(at: index), url: url(at: index))
    }
