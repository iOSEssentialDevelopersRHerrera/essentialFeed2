...
class FeedImageDataLoaderCacheDecoratorTests: XCTestCase, FeedImageDataLoaderTestCase {
    ...
    func test_cancelLoadImageData_cancelsLoaderTask() {
        ...
        XCTAssertEqual(loader.cancelledURLs, [url], "Expected to cancel URL loading from laoder")
    }
    ...
    func test_loadImageData_cahcesLoadedDataOnLoaderSuccess() {
        let cache = CacheSpy()
        let url = anyURL()
        let imageData = anyData()
    ...
    func test_loadImageData_doesNotCahceDataOnLoaderFailure() {
        let cache = CacheSpy()
        let url = anyURL()
        let (sut, loader) = makeSUT(cache: cache)
