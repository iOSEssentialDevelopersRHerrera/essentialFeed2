    ...
    func test_retrieve_hasNoSideEffectsOnNonEmptyCache() {
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { firstResult in
                ...
                default:
                    XCTFail("Expected retrieving twice from non empty cache to deliver same foun result with feed \(feed) and timestamo \(timestamp), got \(firstResult) and \(secondResult) instead")
                }
                exp.fulfill()
            }
            }
